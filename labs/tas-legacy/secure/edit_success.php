...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $sql = "UPDATE list SET name = '$username', title = '$title', content = '$content', filename = '$filename' WHERE id = $id";

    $result = $connect_db->query($sql);
    if($result){
        echo "<script>alert('수정 성공!'); location.href='index.php';</script>";
    }else{
        echo "<script>alert('수정 도중 오류가 발생했습니다. 다시 시도해주세요'); history.back();</script>";
    }
    $connect_db->close();   
}
